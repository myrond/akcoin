...
*/		
	...
	$userInfoQ = mysql_query("SELECT id, username, pin, pass, admin, share_count, stale_share_count, shares_this_round, hashrate, api_key, IFNULL(donate_percent, '0') as donate_percent, IFNULL(round_estimate, '0') as round_estimate, anonymous,score FROM webUsers WHERE id = '".$userId."' LIMIT 0,1"); //
	...
	//Get current round scores
       $totalScoreQ = mysql_query("SELECT value FROM settings where setting='score'");
        while ($totalOverallScoreR = mysql_fetch_array($totalScoreQ))
                $totalOverallScore = intval($totalOverallScoreR[0]);


				
			
	//Prevent divide by zero
	if (isset($totalOverallShares) AND isset($totalOverallShares))
{
